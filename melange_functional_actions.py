...
class FunctionalTests(object):  
  ...
  def waitAndEnterText(self, sec, id_type="", elem=""):
    ...
    self.wait(sec)
    if id_type == "id":
      self.Browser.find_element_by_id(self.obj_id[elem]).send_keys\
                                                        (self.obj_val[elem])
    elif id_type == "xpath":
      self.Browser.find_element_by_xpath(self.obj_id[elem]).send_keys\
                                                        (self.obj_val[elem])
    else:
      raise NoSuchElementException   
  ...
  def clearFieldAndEnterData(self, error_elem , elem=""):
    ...
    self.assertTextIn(error_elem)
    self.waitAndClearField(1, elem)
    self.writeTextField("xpath", elem)
 
  def ClearField(self, id_type="", clear_elem=""):
    ...
    """
    if id_type == "id":
      self.Browser.find_element_by_id(self.obj_id[clear_elem]).clear()
    elif id_type == "xpath":
      self.Browser.find_element_by_xpath(self.obj_id[clear_elem]).clear()
    else:
      raise NoSuchElementException   
 
  def clickOn(self, id_type="", click_elem=""):
    ...
    if id_type == "id":
      self.Browser.find_element_by_id(self.obj_id[click_elem]).click()
    elif id_type == "xpath":
      self.Browser.find_element_by_xpath(self.obj_id[click_elem]).click()
    else:
      raise NoSuchElementException
  ...
  def assertText(self, text_elem=""):
    ...
    txt = self.Browser.find_element_by_xpath(self.obj_id[text_elem]).text
    if txt is None:
        msg = "Element %s has no text %s " % (text_elem, txt)
        self.assertError(msg)
    if txt != self.obj_val[text_elem]:
        msg = "Element text should be %s. It is %s."\
                                            %(self.obj_val[text_elem], txt)
        self.assertError(msg)
  ...
  def assertTextIn(self, text_elem):
    ...
    error_msg = self.Browser.find_element_by_xpath(\
                                               self.obj_id[text_elem]).text
    if error_msg is None:
        msg = "Element %s has no text %s " % (text_elem, error_msg)
        self.assertError(msg)
    if error_msg not in self.obj_val[text_elem]:
        msg = "Element text should be %s.  It is %s." % (self.obj_val[\
                                                         text_elem], error_msg)
        self.assertError(msg)

  def waitAndAssertIfDisplayed(self, sec, elem_displayed=""):
    ...
    try:
      self.Browser.find_element_by_xpath\
                                (self.obj_id[elem_displayed]).is_displayed()
      return True
    except:
      return False
  ...
  def fillRandomValue(self, elem=""):
    ...
    N=5
    val = self.obj_val[elem] + ''.join(random.choice(string.ascii_lowercase\
                                             + string.digits) for x in range(N))
    self.wait(1)
    self.ClearField("xpath", elem)
    self.Browser.find_element_by_xpath(self.obj_id[elem]).send_keys(val)
